...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  /**MongDB expects the radius to be in radiants,
   * and we get the radiants by dividing our distance with the radius of the earth
   */
  const radius = unit === 'mi' ? distance / 3963.2 : distance / 6378.1;

  if (!lat || !lng) {
    next(new AppError('Please provide latitude and longitude of your ', 400));
  }
...
exports.getDistances = catchAsync(async (req, res, next) => {
  ...
  const distances = await Tour.aggregate([
    ...
    {
      // This "$project" here allows us to state the properties we want to keep
      $project: {
        distance: 1,
        name: 1,
      },
